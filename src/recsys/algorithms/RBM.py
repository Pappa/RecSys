...
class Recommender(object):
    ...
    def Train(self, X):
        ...
        for epoch in range(self.epochs):
            ...
            for i in range(0, trX.shape[0], self.batchSize):
                self.sess.run(
                    self.update, feed_dict={self.X: trX[i : i + self.batchSize]}
                )
    ...
    def GetRecommendations(self, inputUser):
        ...
        visible = tf.nn.sigmoid(
            tf.matmul(hidden, tf.transpose(self.weights)) + self.visibleBias
        )
    ...
    def MakeGraph(self):
        ...
        vMask3D = tf.reshape(
            vMask, [tf.shape(v)[0], -1, self.ratingValues]
        )  # Reshape into arrays of individual ratings
        vMask3D = tf.reduce_max(
            vMask3D, axis=[2], keepdims=True
...
class RBM(AlgoBase):
    ...
    def buildStoplist(self, trainset):
        ...
        for iiid in trainset.all_items():
            ...
            title = self.ml.get_movie_name(int(movieID))
            if title:
                title = title.lower()
