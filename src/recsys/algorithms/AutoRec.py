...
class Recommender:
    ...
    def initialize_weights_biases(self):
        ...
        self.weights = {
            "h1": tf.Variable(
                tf.random.normal([self.visible_dimensions, self.hidden_dimensions])
            ),
            "out": tf.Variable(
                tf.random.normal([self.hidden_dimensions, self.visible_dimensions])
            ),
        }
    ...
    def neural_net(self, input_user):
        ...
        hidden = tf.nn.sigmoid(
            tf.add(tf.matmul(self.input_layer, self.weights["h1"]), self.biases["b1"])
        )

        # output layer for our predictions.
        self.output_layer = tf.nn.sigmoid(
